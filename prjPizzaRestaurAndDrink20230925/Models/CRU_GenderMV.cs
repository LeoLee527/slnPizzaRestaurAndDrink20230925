...
namespace prjPizzaRestaurAndDrink20230925.Models
{
    public class CRU_GenderMV
    {
        ...
        {
            GetAllGenders();
            var editgender = new PizzaRestaurantAndDrinkDbEntities().GenderTables.Where(g => g.GenderID == id).FirstOrDefault();
            if (editgender != null)
            {
        ...
        private void GetAllGenders()
        {
            ...
            foreach (var gender in new PizzaRestaurantAndDrinkDbEntities().GenderTables.ToList())
            {
